...
function generateGame(seed, timeLimit, options = { mode: "RANDOM", moduleCount: 5, selectedModules: [] }) {
  ...
  if (options.mode === "MANUAL") {
    for (const type of options.selectedModules) {
      ...
      switch (type) {
        case "WIRES":
          mod = generateWires(rng, modules.length, globalProps, rules);
          break;
        case "BUTTON":
          mod = generateButton(rng, modules.length, globalProps);
          break;
        case "SIMON":
          mod = generateSimon(rng, modules.length, globalProps, rules);
          break;
        case "KNOB":
          mod = generateKnob(rng, modules.length, globalProps, rules);
          break;
        case "LOGIC":
          mod = generateLogic(rng, modules.length, globalProps, rules);
          break;
      }
...
function calculateGlobalRules(rng, props) {
  ...
  const wireRuleDefinitions = [
    ...
    { id: "no_yellow", text: "jeżeli nie występuje żaden przewód żółty", check: (w) => !w.includes("żółty"), action: { text: "przetnij pierwszy przewód", index: 0 } },
    { id: "last_red", text: "jeżeli ostatni przewód jest koloru czerwonego", check: (w) => w[w.length - 1] === "czerwony", action: { text: "przetnij czwarty przewód", index: 3 } },
    { id: "more_1_blue", text: "jeżeli występuje więcej niż jeden przewód niebieski", check: (w) => w.filter((c) => c === "niebieski").length > 1, action: { text: "przetnij ostatni niebieski przewód", index: -1 } }
  ];
...
function generateManual(rng, rules) {
  const pages = [];
  let wiresHtml = `
    ...
    `;
  rules.wires.activeRules.forEach((rule, i) => {
    wiresHtml += `
            <li>
...
function generateWires(rng, index, props, rules) {
  const wireCount = 4;
  const wires = [];
...
function generateButton(rng, index, props, rules) {
...
function generateSimon(rng, index, props, rules) {
...
function generateKnob(rng, index, props, rules) {
  const KNOB_POSITIONS = ["GÓRA", "DÓŁ", "LEWO", "PRAWO"];
...
function generateLogic(rng, index, props, rules) {
